...
> **重要**：在繼續實作前，請先閱讀 [OPTIMIZATION_REVIEW.md](OPTIMIZATION_REVIEW.md)

經過深度檢視，發現目前的重構實作存在**過度設計**問題：

| 問題           | 嚴重度 | 說明                                            |
| -------------- | ------ | ----------------------------------------------- |
| I/O 效率低落   | 🔴 高   | 每個函式各自讀寫工作表，Pipeline 執行 6+ 次 I/O |
| 過度抽象       | 🟡 中   | `createStatisticsContainer()` 多數功能未使用    |
| 資料來源不一致 | 🟡 中   | session.students vs classroom.students          |

### 建議優化方向

1. **改為 Pipeline 模式**：單次讀取 → 多函式處理 → 單次寫回
2. **簡化統計容器**：移除通用建構器，改為直接 getter
3. **統一資料來源**：決定從 session 或 classroom 收集

### 預期效益

- 執行時間減少 50%（I/O 次數從 6+ 降為 2）
- 程式碼減少 30%（約 400 行）
- 維護難度降低
